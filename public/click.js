...
function ButtonCtrl($scope, $window, buttonApi) {
	...
	$scope.reload = reload;
	$scope.receipt = [];
  $scope.emptyreceipt = emptyreceipt;
  $scope.totalreceipt = totalreceipt

  function totalreceipt(list) {
  ...
  function emptyreceipt(){
    console.log("Closeing the receipt")
    $scope.receipt = [];
  }
	...
	function completeTransaction() {
		...
		buttonApi.completeTransaction(user, totalPrice)
			.success(function () {
				$scope.receipt = $scope.transactionItems;
				$scope.totalPrice = 0;
				$scope.transactionItems = [];
				refreshButtons();
				listTransaction();
